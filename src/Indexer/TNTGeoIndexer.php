...
class TNTGeoIndexer extends TNTIndexer
{
    ...
    public function createIndex(string $indexName)
    {
        ...
        $this->engine->index = new PDO(
            "sqlite:" . $this->engine->config["storage"] . $indexName,
        );
        $this->engine->index->setAttribute(
            PDO::ATTR_ERRMODE,
        ...
        $this->engine->index->exec(
            "CREATE INDEX location_index ON locations ('longitude', 'latitude');",
        );

        $this->engine->index->exec(
            "CREATE TABLE IF NOT EXISTS info (key TEXT, value INTEGER)",
        );
    ...
    public function prepareInsertStatement()
    {
        ...
        $this->insertStmt = $this->engine->index
            ->prepare("INSERT INTO locations (doc_id, longitude, latitude, cos_lat, sin_lat, cos_lng, sin_lng)
    ...
    {
        $this->engine->prepareAndExecuteStatement(
